...
const mapSchema = z.object({
    ...
    description: z.string().min(1, { message: "Please enter a map description." }).trim(),
...
export async function editMap(id: number, state: FormState, formData: FormData) {
    const inputData = {
    ...
    try {
        const map = await prisma.map.update({
            ...
            data: {
                title, description, ownerId: user.id, published: published === 'on', updatedAt: new Date(Date.now())
            }
        })
